...
<template>
	<view>
		<cu-custom bgColor="bg-gradual-blue" :isBack="true">
			...
			<block slot="content">{{show == 1 ? 'Task' : '需求制定' }}</block>
		</cu-custom>
		...
		<view class="cu-timeline" v-if="show == 1">
			<view class="flex">
				<view class="flex-sub padding-sm margin-xs radius">
					<view class="grid margin-bottom text-center col-1" >
					    <view class="text-gray text-left">{{currentDay}} {{currentMonth}}</view>
						<view class="text-sl text-left bold">Today</view>
					</view>
				</view>
			...
			<scroll-view scroll-x class="bg-white nav text-center" id="scroll">
				<view class="cu-item" :class="index==TabCur?'text-blue cur':''" v-for="(item,index) in 7" :key="index" @tap="tabSelect" :data-id="index">
					<view class="text-gray ">{{week[index]}}</view>
				</view>
			</scroll-view>


			<view class="cu-time" v-if="studentCoursesByWeek.length">
...
<!-- 					<view class="cu-capsule radius">
						...
						<view class="cu-tag line-blue bg-white" >web</view>
					</view> -->
					<view class="cu-capsule radius">
						<view v-if="item.isMorning" class="cu-tag bg-cyan">上午</view>
						<view v-else class="cu-tag bg-cyan">下午</view>

						<view class="cu-tag line-cyan bg-white">{{item.course_start_time}}-{{item.course_end_time}}</view>
					</view>
					<view class="margin-top text-xxl">{{item.course_name}}</view>				
					<view class="margin-top-sm">
						<view class="cuIcon-peoplefill">{{'&nbsp;'}}{{item.teacher_name}}</view>
					</view>
					<view class="margin-top-sm">
						<view class="cuIcon-read">{{'&nbsp;'}}{{item.classroom_name}}</view>
					</view>
				</view>
			</view>
...
<!-- 			<view class="cu-time">
			...
			<view class="cu-time" style="padding-bottom: 140rpx;" v-if="studentCoursesByWeek.length">
				<text class='cuIcon-selection text-white text-lg bg-blue round padding-xs'></text>
		...
		<!-- 资讯按钮 -->
		<view class="cu-bar bg-white tabbar shop shadow edit-fixed" v-if="false">
		...
		methods: {
			getHeight(){
				// 使用 wx.createSelectorQuery 创建查询对象
				  ...
				  // 执行查询并获取位置信息
				  query.exec((res) => {
				    // res 中包含了标签 scroll-view 的位置信息
				    var scrollViewBottom = res[0].bottom;
				
				    // 获取屏幕高度
				    var windowHeight = wx.getSystemInfoSync().windowHeight;
				
				    // 计算最下边到屏幕最下方的距离
				    var distanceToBottom = windowHeight - scrollViewBottom;
					
					this.emptyHight = (distanceToBottom*2).toString();
				})
			},
			tabSelect(e) {
			...
			changeStudentCourse(){
				...
				this.studentCoursesByWeek.forEach(course => {
					...
					var courseEndTime = course.course_end_time;
			
					// 将 course_end_time 转换为 Date 对象
					var endTimeParts = courseEndTime.split(":");
					var endHour = parseInt(endTimeParts[0], 10);
			
					// 判断是否是上午，然后添加 ismorning 属性
					course.isMorning = endHour < 12;
				});
			},
			getData() {
				let opts = {
				...
				request.httpRequest(opts).then(res => {
					...
					if (res.statusCode == 200) {					
						this.studentCourses = res.data.studentCourses
						this.changeStudentCourse();
					} else {
						console.log('error!');
					}
