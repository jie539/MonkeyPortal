		...
		<form class="cl">
			<view class="t-a">
				<picker @change="PickerChange" :value="index" :range="picker">
					<view class="area-code">+{{picker[index]}}</view>
				</picker>
			...
			<view class="t-a">
				...
				<view v-if="showText" class="t-c" @tap="send()">发送</view>
				<view v-else class="t-c" style="background-color: #A7A7A7;">重新发送({{ second }})</view>
			</view>
			<button @tap="verify()">提 交</button>
		</form>
		<view class="t-f"><text>————— 返回登录 —————</text></view>
		<view class="t-e cl">
	...
	export default {
		...
		methods: {
			...
			sendCode() {
				...
				const userInfo = {
					email: that.email,
					telephone: that.picker[that.index] + that.phone,
				}
